...
export const ChatWindow = ({
  ...
  const getChatHeader = React.useCallback(() => {
    ...
    return (
      <Box>
        ...
        >
          <HStack
            className={styles.metaItemGroup}
            fontSize="sm"
            color="gray.500"
          >
  ...
  return (
    <Box w="calc(100% - 450px)" h="100%" position="relative">
            ...
            return (
              <Box key={message.ts.toString()}>
                {message.type === 'MANUAL' ? (
                  <ChatBubble
                    message={message.msg}
                    type={message.sender === 'admin' ? 'bot' : 'user'}
                  />
                ) : (
                  <>
                    <ChatBubble
                      message={message.q || message.msg}
                      type={'user'}
                    />
                    <ChatBubble
                      message={message.a || message.msg}
                      type={'bot'}
                    />
                  </>
                )}
      ...
      {chatData && (permissions.get().isAdmin || permissions.get().isEditor) ? (
        <Box
