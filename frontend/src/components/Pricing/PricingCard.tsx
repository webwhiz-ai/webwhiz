...
export const PricingCard = ({
  ...
  const getPricingMarkup = React.useCallback(
    (type: SubscriptionType | SubscriptionTypeLTD) => {
      let ltdPrice = 0;
      switch (type) {
        case 'AppSumo tier 1':
  ...
  const getPaymentLink = React.useCallback(() => {
    const subscriptionLinks = {
      Standard: {
        YEARLY: `https://webwhiz.lemonsqueezy.com/checkout/buy/07f55c4e-19ff-47e5-b2d0-6fad6a0172fa?checkout[email]=${userEmail}${
          discountData?.couponCode
            ? `&checkout[discount_code]=${discountData?.couponCode}`
            : ''
        }`,
        MONTHLY: `https://webwhiz.lemonsqueezy.com/checkout/buy/40a6cfdf-60b5-402f-a071-5ff008834838?checkout[email]=${userEmail}${
          discountData?.couponCode && false
            ? `&checkout[discount_code]=${discountData?.couponCode}`
            : ''
        }`,
      },
      Premium: {
        YEARLY: `https://webwhiz.lemonsqueezy.com/checkout/buy/521d2310-353e-4b93-b15e-5bda78c0fa74?checkout[email]=${userEmail}${
          discountData?.couponCode
            ? `&checkout[discount_code]=${discountData?.couponCode}`
            : ''
        }`,
        MONTHLY: `https://webwhiz.lemonsqueezy.com/checkout/buy/2c16a858-6699-4aae-b6e7-55da6797f975?checkout[email]=${userEmail}${
          discountData?.couponCode && false
            ? `&checkout[discount_code]=${discountData?.couponCode}`
            : ''
        }`,
      ...
      Base: {
        YEARLY: `https://webwhiz.lemonsqueezy.com/checkout/buy/70ec6666-18ba-4ebf-9c10-c851ab7531e5?checkout[email]=${userEmail}${
          discountData?.couponCode
            ? `&checkout[discount_code]=${discountData?.couponCode}`
            : ''
        }`,
        MONTHLY: `https://webwhiz.lemonsqueezy.com/checkout/buy/11c44a3d-7223-45b0-97d3-75d921150f67?checkout[email]=${userEmail}${
          discountData?.couponCode && false
            ? `&checkout[discount_code]=${discountData?.couponCode}`
            : ''
        }`,
      ...
      Enterprise: {
        YEARLY: `https://webwhiz.lemonsqueezy.com/checkout/buy/71a30d03-079d-43b7-9b59-76c3c5e378e1?checkout[email]=${userEmail}${
          discountData?.couponCode
            ? `&checkout[discount_code]=${discountData?.couponCode}`
            : ''
        }`,
        MONTHLY: `https://webwhiz.lemonsqueezy.com/checkout/buy/817141ba-5ce0-4c70-bfd1-223dc8a8b30b?checkout[email]=${userEmail}${
          discountData?.couponCode && false
            ? `&checkout[discount_code]=${discountData?.couponCode}`
            : ''
        }`,
  ...
  const getPricingBtn = React.useCallback(() => {
    ...
    return (
      <Box mt={6}>
        <Link href={getPaymentLink()} isExternal>
          <Button
            variant="solid"
  ...
  return (
    ...
    >
      ...
      <Flex mb={4}>{getPricingMarkup(subscriptionData.type)}</Flex>

      <List spacing={3}>
        <ListItem>
          <ListIcon
            boxSize={22}
            as={CheckCircleBlueIconMd}
            color={isPopular ? 'white' : 'blue.500'}
          />
