...
export const ChatSessions = ({ chatData, handlePageClick, isChatLoading }) => {
    ...
    <Flex w="100%">
      <Box className={styles.listCont} pos="relative">
        <Box className={styles.listContInner} pos="relative">
          ...
          {chatData.results
            .filter((data) => data.firstMessage)
            .map((data) => (
              <Box
                className={classNames(styles.listTab, {
                  [styles.activeList]:
                    selectedChatData && selectedChatData._id === data._id,
                })}
                shadow={'xs'}
                key={data._id}
                p="3"
      ...
      <Box w="50%" className={styles.chatDetailsCont}>
        ...
        {chatDetails &&
          ...
          chatDetails.messages.map((data) => {
            return (
              <Box key={data.ts}>
                <Box
                  className={classNames(
                    styles.detailsQuestion,
                    styles.detailsItem,
                  )}
                >
                  <div className={styles.detailsItemInner}>{data.q}</div>
                </Box>
                <Box
                  className={classNames(
                    styles.detailsAnswer,
                    styles.detailsItem,
                  )}
                >
                  <div className={styles.detailsItemInner}>{data.a}</div>
                </Box>
              </Box>
            );
          })}
