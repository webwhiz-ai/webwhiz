...
export const ChatSessionsNew = ({ chatbotId, userId }: ChatSessionsProps) => {
  ...
  const [isChatSessionsLoading, setIsChatSessionsLoading] =
    useState<boolean>(true);
  const [isChatDataLoading, setIsChatDataLoading] = useState(true);

  const addManualMessage = useCallback(
    (chatData: ChatSessionDetail, data: Partial<MessageList>) => {
  ...
  const onChatReplyEvent = useCallback(
    (sessionId: string, msg: string) => {
      const data = { sender: 'admin', sessionId, msg };
      if (chatData) {
        const updatedChatData = addManualMessage(chatData, {
          ...data,
          type: 'MANUAL',
        });
  ...
  const updateChatSessionReadStatus = useCallback(
    async (
      chatId: string,
      isUnread: boolean,
      needStateUpdate: boolean = true,
    ) => {
      try {
  ...
  const onDeleteChat = useCallback(
    async (chatId: string) => {
      try {
        ...
        if (chatSessions) {
          const updatedResults = chatSessions.results.filter(
  ...
  const onChatEvent = useCallback(
    (data: MessageList) => {
      ...
      setChatSessions((prev) => {
        let chatSession = prev?.results.find(
          (item) => item._id === data.sessionId,
        );
        if (chatSession) {
          data?.sessionId !== chatData?._id &&
            !chatSession.isUnread &&
            updateChatSessionReadStatus(chatSession?._id, true, false);
        ...
        return {
          ...
          results: [
            chatSession,
            ...(prev?.results.filter((item) => item._id !== data.sessionId) ||
              []),
          ],
        };
      });
  ...
  const onNewSessionEvent = useCallback(async (data: MessageList) => {
    ...
    setChatSessions((prev) => {
      const prevResults = prev?.results.find(
        (item) => item._id === data.sessionId,
      );
      if (prevResults) {
        return prev;
      ...
      return {
        ...prev,
        results: [newSession, ...(prev?.results || [])],
      };
    });
  }, []);

  const onNewBotReply = useCallback(
    (data: MessageList) => {
  ...
  const selectedChat = chatSessions?.results.find(
  ...
  return (
    <Flex w="100%">
      ...
      />
      <ChatWindow
        onChatReply={onChatReplyEvent}
        chatData={chatData}
        userData={chatData?.userData}
        messages={chatData?.messages}
        isMessagesLoading={isChatDataLoading}
      />
    </Flex>
  );
};
