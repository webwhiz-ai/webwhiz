...
export const CustomDomain = ({
  ...
  const handleCustomDomainSave = React.useCallback(
    async (values) => {
  ...
  const [recordName, setRecordName] = React.useState<string>(
    defaultCustomDomain ? defaultCustomDomain.split('.')[0] : '',
  );

  const [domainName, setDomainName] =
    React.useState<string>(defaultCustomDomain);
  ...
  const handleValueCopy = React.useCallback(() => {
    navigator.clipboard.writeText('widget.webwhiz.ai.');
    toast({
      title: `Copied to Clipboard`,
      status: 'info',
      isClosable: true,
    ...
    <Box maxW="620px">
                  ...
                  {({ field, form }: any) => (
                    ...
                    >
                      ...
                      <FormHelperText fontSize="smaller" color="gray.400">
                        Only enter the domain name. Do not include additional
                        path or protocol
                      </FormHelperText>
      ...
      <HStack>
        <FormControl>
          ...
          <InputGroup size="md">
            <Input readOnly value={recordName} />
        ...
        <FormControl>
          ...
          <InputGroup size="md">
            <Input readOnly value={'widget.webwhiz.ai.'} />
            <InputRightElement width="4.5rem">
      ...
      <Box>
        <OrderedList fontSize="sm" mt="4" pl="1" mb="2">
          ...
          <ListItem>
            In the 'Name' or 'Host' field, input your chosen subdomain. Copy
            from the 'Name' field above.
          </ListItem>
          <ListItem>
            In the 'Value' or 'Target', use widget.webwhiz.ai. Make sure it's
            typed correctly for the right domain link.
          </ListItem>
          <ListItem>
            After adding the record, click on the button link below. We'll then
