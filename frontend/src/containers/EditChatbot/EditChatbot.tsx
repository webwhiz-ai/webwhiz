...
const EditChatbot = (props: EditChatbotProps) => {
  ...
  const [chatBot, setChatbot] = React.useState<Knowledgebase>(
    {} as Knowledgebase,
  );
  ...
  const [crawlDataLoading, setIrawlDataLoading] =
  ...
  const getDocsDataPagination = React.useCallback(
    async (pageNo: number) => {
      try {
  ...
  const startEmbeding = React.useCallback(
    (chatBotId) => {
      let interval = setInterval(async () => {
        ...
        if (
          ...
          setProductSetupLoadingText(
            'Training ChatGPT with your website data... This may take some time based on the amount of the data...',
          );
  ...
  const handleChatbotUpdate = React.useCallback(
    async (formValues) => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const userRole = response.data.participants.find(
        ...
        if (
          chatBotData.status === 'EMBEDDING_ERROR' ||
          chatBotData.status === 'CRAWL_ERROR'
        ) {
        ...
        <VStack alignItems="start">
          ...
          <Box className={styles.codeBlock}>
            <span style={{ color: '#808080' }}>
              ...
              <span style={{ color: '#183691' }}>"__webwhizSdk__"</span>
              <span style={{ color: '#0086b3', paddingLeft: '4px' }}>
                {' '}
                data-chatbot-id
              </span>
              =<span style={{ color: '#183691' }}>"{chatBot._id}"</span>
              <br></br>
              ...
              <span style={{ color: '#183691' }}>
                "https://widget.webwhiz.ai/webwhiz-sdk.js"
              </span>
              &gt;
            </span>
            <br></br>
        ...
        <VStack alignItems="start">
          ...
          <Box className={styles.codeBlock}>
            <span style={{ color: '#808080' }}>
              ...
              =
              <span style={{ color: '#183691' }}>
                "https://widget.webwhiz.ai/?kbId={chatBot._id}
                &embed=true&hide-chat-actions=true"
              </span>
              &gt;
            </span>
            <br></br>
    ...
    return (
      <>
          ...
          >
            {customTrainingDataPage?.results.map((data) => (
              <Box
                ...
                bg={
                  selectedTrainingData._id === data._id ? 'gray.100' : 'white'
                }
                borderRight={
                  selectedTrainingData._id === data._id ? '2px' : '0'
                }
                ...
                className={classNames(styles.questionTab, {
                  [styles.activeQuestion]:
                    ...
                    selectedTrainingData._id === data._id,
                })}
                key={data._id}
                p="3"
              >
                ...
                <IconButton
                  ...
                  size="xs"
                  isLoading={
                    deleteCustomDataLoading && questionsToDelete === data._id
                  }
                  onClick={() => {
        ...
        >
          ...
          {selectedTrainingData && (
            <AddTrainingDataForm
              knowledgeBaseId={props.match.params.chatbotId}
  ...
  const getDefaultCustomizationValues = React.useCallback(() => {
    ...
    return {
      ...
      showReadMore:
        chatBot.chatWidgeData?.showReadMore === undefined
          ? chatWidgetDefaultValues.showReadMore
          : chatBot.chatWidgeData?.showReadMore,
      showAsPopup:
        chatBot.chatWidgeData?.showAsPopup === undefined
          ? chatWidgetDefaultValues.showAsPopup
          : chatBot.chatWidgeData?.showAsPopup,
      popupDelay:
        chatBot.chatWidgeData?.popupDelay || chatWidgetDefaultValues.popupDelay,
      collectEmailText:
        chatBot.chatWidgeData?.collectEmailText ||
      ...
      customCSS:
        chatBot.chatWidgeData?.customCSS || chatWidgetDefaultValues.customCSS,
      questionExamples:
        chatBot.chatWidgeData?.questionExamples ||
        chatWidgetDefaultValues.questionExamples,
      welcomeMessages: chatBot.chatWidgeData?.welcomeMessage
        ? [chatBot.chatWidgeData?.welcomeMessage]
        : chatBot.chatWidgeData?.welcomeMessages ||
      ...
      launcherIcon:
        chatBot.chatWidgeData?.launcherIcon ||
        chatWidgetDefaultValues.launcherIcon,
      chatInputPlaceholderText:
        chatBot.chatWidgeData?.chatInputPlaceholderText ||
        chatWidgetDefaultValues.chatInputPlaceholderText,
      assistantTabHeader:
        chatBot.chatWidgeData?.assistantTabHeader ||
        chatWidgetDefaultValues.assistantTabHeader,
      offlineMsgTabHeader:
        chatBot.chatWidgeData?.offlineMsgTabHeader ||
        chatWidgetDefaultValues.offlineMsgTabHeader,
      readMoreText:
        chatBot.chatWidgeData?.readMoreText ||
        chatWidgetDefaultValues.readMoreText,
      offlineMsgHeading:
        chatBot.chatWidgeData?.offlineMsgHeading ||
        chatWidgetDefaultValues.offlineMsgHeading,
      offlineMsgDescription:
        chatBot.chatWidgeData?.offlineMsgDescription ||
        chatWidgetDefaultValues.offlineMsgDescription,
      nameFieldLabel:
        chatBot.chatWidgeData?.nameFieldLabel ||
        chatWidgetDefaultValues.nameFieldLabel,
      nameFieldPlaceholder:
        chatBot.chatWidgeData?.nameFieldPlaceholder ||
        chatWidgetDefaultValues.nameFieldPlaceholder,
      emailFieldLabel:
        chatBot.chatWidgeData?.emailFieldLabel ||
        chatWidgetDefaultValues.emailFieldLabel,
      emailFieldPlaceholder:
        chatBot.chatWidgeData?.emailFieldPlaceholder ||
        chatWidgetDefaultValues.emailFieldPlaceholder,
      msgFieldLabel:
        chatBot.chatWidgeData?.msgFieldLabel ||
        chatWidgetDefaultValues.msgFieldLabel,
      msgFieldPlaceholder:
        chatBot.chatWidgeData?.msgFieldPlaceholder ||
        chatWidgetDefaultValues.msgFieldPlaceholder,
      requiredFieldMsg:
        chatBot.chatWidgeData?.requiredFieldMsg ||
        chatWidgetDefaultValues.requiredFieldMsg,
      invalidEmailMsg:
        chatBot.chatWidgeData?.invalidEmailMsg ||
        chatWidgetDefaultValues.invalidEmailMsg,
      formSubmitBtnLabel:
        chatBot.chatWidgeData?.formSubmitBtnLabel ||
        chatWidgetDefaultValues.formSubmitBtnLabel,
      enableHumanChat:
        chatBot.chatWidgeData?.enableHumanChat ||
        chatWidgetDefaultValues.enableHumanChat,
      formSubmitBtnSubmittingText:
        chatBot.chatWidgeData?.formSubmitBtnSubmittingText ||
        chatWidgetDefaultValues.formSubmitBtnSubmittingText,
      formSubmitSuccessMsg:
        chatBot.chatWidgeData?.formSubmitSuccessMsg ||
        chatWidgetDefaultValues.formSubmitSuccessMsg,
      formSubmitErrorMsg:
        chatBot.chatWidgeData?.formSubmitErrorMsg ||
        chatWidgetDefaultValues.formSubmitErrorMsg,
      formSendAgainBtnLabel:
        chatBot.chatWidgeData?.formSendAgainBtnLabel ||
        chatWidgetDefaultValues.formSendAgainBtnLabel,
      formTryAgainBtnLabel:
        chatBot.chatWidgeData?.formTryAgainBtnLabel ||
        chatWidgetDefaultValues.formTryAgainBtnLabel,
  ...
  const getChatSessionsComponent = React.useCallback(() => {
    ...
    return (
      <ChatSessionsNew
        ...
        chatbotId={props.match.params.chatbotId}
      />
    );
  ...
  const getMainComponent = React.useCallback(() => {
    ...
    return (
      <>
        {access.isAdmin || access.isEditor ? (
          ...
          >
            <ChatBotProductSetup
              ...
              onPrimaryBtnClick={(
                formValues: ProductSetupData,
                hasWebsiteDataChanged: boolean,
              ) => {
          ...
          >
            <VStack alignItems="start" w="100%" position="relative">
              <SectionTitle
                ...
                description="Improve your chatbot by adding more training data. The more you add, the better its responses. Group logically & avoid lengthy inputs."
              />
        ...
        >
          ...
          {chatBot._id ? (
            <CustomDomain
              defaultCustomDomain={chatBot.customDomain}
          ...
          >
            ...
            <HStack w="100%">
              <Box w="50%">
                <ChatBot
                  knowledgeBaseId={chatBot._id}
                  customStyle={chatBot.chatWidgeData}
              ...
              <Box w="50%" pos="relative" h="100%">
                <Flex className={styles.resultTips} direction="column">
                  ...
                  <Box className={styles.resultTip}>
                    ...
                    <Heading
                      className={styles.resultTipHeading}
                      fontSize="medium"
                    >
                      Add more training Data
                    </Heading>
                    <Text className={styles.resultTipDescg}>
                      The more data you add, the better the chatbot response
                      will be. You can add custom data to train in the form of
                  ...
                  <Box className={styles.resultTip}>
                    ...
                    <Text className={styles.resultTipDescg}>
                      verify that all the necessary pages on your website are
                      crawled correctly. To do this, head to the{' '}
                      <Text
                        as="span"
                        cursor="pointer"
                        textDecoration="underline"
                  ...
                  <Box className={styles.resultTip}>
                    ...
                    <Text className={styles.resultTipDescg}>
                      ...
                      make sure to train your chatbot with past support
                  ...
                  <Box className={styles.resultTip}>
                    ...
                    <Text className={styles.resultTipDescg}>
                      ...
                      or schedule a{' '}
                      <a
                        href="https://tidycal.com/neravath/webwhiz-15-minute-meeting"
                        target="_blank"
                        style={{ textDecoration: 'underline' }}
                      >
                        meeting.
    ...
    chatBot._id,
    chatBot.websiteData?.websiteUrl,
    chatBot.customDomain,
    chatBot.participants,
    chatBot.chatWidgeData,
    access.isAdmin,
    access.isEditor,
    access.isOwner,
    currentStep,
    getCrawlDataPagination,
    getDocsDataPagination,
    getExcludedPaths,
    getIncludedPaths,
    handleTabChange,
    defaultCrauledData,
    ...
    primaryButtonLabel,
    getDefaultCustomizationValues,
    user?.subscriptionData?.name,
    getAddToWebsiteContent,
    props.match.params.chatbotId,
    handleTrainingDataSave,
    getCustomDataComponent,
    getChatSessionsComponent,
    offlineMessages,
    isChatLoading,
    handleOfflinePageClick,
    history,
    handleChatbotUpdate,
    toast,
    goToStep,
      ...
      >
        <Link to="/app/chat-bots/">
          <Flex alignItems="center">
            ...
            <Heading ml="2" color="white" fontSize="14" fontWeight="400">
              Chatbots
            </Heading>
          </Flex>
        </Link>
            ...
            <List spacing={2}>
              {access.isAdmin || access.isEditor ? (
                <ListItem
                  ...
                  fontSize="md"
                  className={
                    currentStep === 'product-setup' ? styles.active : ''
                  }
                  cursor="pointer"
                  onClick={() => {
                    goToStep('product-setup');
              ...
              {access.isAdmin || access.isEditor ? (
                <ListItem
                  ...
                  fontSize="md"
                  className={
                    currentStep === 'train-custom-data' ? styles.active : ''
                  }
                  cursor="pointer"
                  onClick={() => {
                    goToStep('train-custom-data');
              ...
              <ListItem
                ...
                fontSize="md"
                className={
                  currentStep === 'offline-messages' ? styles.active : ''
                }
                cursor="pointer"
                onClick={() => {
              ...
              {access.isAdmin || access.isEditor ? (
                <ListItem
                  ...
                  className={
                    currentStep === 'custom-domain' ? styles.active : ''
                  }
                >
                  <svg
                    width="24"
