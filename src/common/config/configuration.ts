...
const config = (): AppConfig => {
  ...
  // Iterate over the environment variables and initialize feature flags
  Object.keys(process.env).forEach((key) => {
    if (key.startsWith('FF_')) {
      const featureFlagName = key.replace('FF_', '').toLowerCase();
      featureFlags[featureFlagName] = process.env[key] === 'true';
    }
  });
